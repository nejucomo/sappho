...
pub fn test_eval(inpath: PathBuf, input: &str, expected: &str) {
    ...
    let res = interpret(codor(inpath, input));
...
pub fn test_unparse(inpath: PathBuf, input: &str, expected: &str, style: &str) {
    ...
    let ast = parse(codor(inpath, input)).unwrap();
...
fn codor(inpath: PathBuf, input: &str) -> CodeOrigin<'_> {
    CodeOrigin::new(input, format!("file {:?}", inpath.display()))
}
