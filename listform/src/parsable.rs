...
impl<X, T, PX, PT> ParsableWith<(PX, PT)> for ListForm<X, T>
where
    ...
    PT: Parser<T> + Clone,
{
    fn make_parser_with((px, pt): (PX, PT)) -> impl Parser<Self> {
        let tailmatch = || just("..").ignore_then(pt.clone());
        let nonempty_body = px.separated_by(just(',').then_opt_space()).at_least(1);
